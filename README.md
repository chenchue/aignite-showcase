...
- **Deliberate MVP scope, with the seams already in.** A few capabilities are simplified for this demo, each behind an interface so the fuller version drops in without changing the layers above it. The single demo subject fits in one prompt, so there's no retrieval layer yet; a multi-subject product at scale is exactly where RAG slots in. Spaced repetition is a `pass → ×2 / fail → 1-day` rule rather than FSRS, enough to show resurfacing while keeping the scheduler easy to reason about and debug. Per-strategy session behaviour is wired end to end but intentionally inert. None of these is a dead end: each is a deliberate boundary with a clean upgrade path, [documented](ARCHITECTURE.md#10-what-was-deliberately-not-built) with its reasoning.
