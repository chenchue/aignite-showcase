...
- **Two cooperating agents, explicitly orchestrated.** A **Tutor** agent runs the Prime → Teach → Assess loop, then hands the answer to a separate **Evaluator** agent that scores it. They're two separate agents (their own prompts, temperatures, and output schemas, on a shared base model) wired as nodes in a small [LangGraph](https://github.com/langchain-ai/langgraph) state machine, with explicit conditional routing. A bare `while` loop would have hidden the structure the design is about; a multi-subgraph framework would have been over-engineering for two agents. The Evaluator's per-level diagnosis (a knowledge gap, or an evident misconception) is fed back into the next explanation and question, which is where the adaptation actually lives.
- **Tutoring turns are durable and resumable.** A turn pauses mid-graph to wait for the student's answer and resumes from a SQLite checkpoint, so a session survives a page reload or a server restart with no in-memory state. A guessed session id reads nothing: ownership is re-derived from the auth token against the checkpoint.
...
- **Deliberate MVP scope, with the seams already in.** A few capabilities are simplified for this demo, each behind an interface so the fuller version drops in without changing the layers above it. The single demo subject fits in one prompt, so there's no retrieval layer yet; a multi-subject product at scale is exactly where RAG slots in. Spaced repetition is a `pass → ×2 / fail → 1-day` rule rather than FSRS, enough to show resurfacing while keeping the scheduler easy to reason about and debug. Per-strategy session behaviour is wired end to end but intentionally inert. None of these is a dead end: each is a deliberate boundary with a clean upgrade path, [documented](ARCHITECTURE.md#12-out-of-mvp-scope-deferred-by-design) with its reasoning.
